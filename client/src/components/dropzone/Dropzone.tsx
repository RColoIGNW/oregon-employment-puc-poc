...
export default function Dropzone(props: { applicationId: string }) {
  ...
  const { handleSubmit, removeFile, files } = useFileUpload({
          ...
          name.toLowerCase().includes(".jpeg") ? (
            <div className={classes.thumb}>
              <div className={classes.thumbInner}>
                <img src={file.imgUrl} className={classes.img} alt={name} />
              </div>
            </div>
          ) : (
            <span>{name}</span>
  ...
  return (
    <div className="container">
      <Container
        {...(getRootProps({ isDragActive, isDragAccept, isDragReject }) as any)}
