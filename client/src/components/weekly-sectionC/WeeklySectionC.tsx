...
export default (props: WeeklySectionProps) => {
  ...
  const [workSearchRecords, setWorkSearchRecords] = useState<
    WorkSearchRecord[]
  >(props.application.workSearchRecords || [])
  ...
  const updateEmploymentRecord = (workSearchRecord: WorkSearchRecord) => {
    const index = workSearchRecords.findIndex(
      (r) => r.id === workSearchRecord.id
    )
  ...
  return (
    <Grid container={true} spacing={2} direction="column">
          ...
          <Grid style={{ display: "flex", paddingLeft: 10, paddingBottom: 15 }}>
            ...
            {expandedDescription && (
              <HelpDialog
                ...
                textSections={questions.helpDialog}
                openFn={setOpen}
              />
            )}
          </Grid>
      ...
      <Grid container={true} direction={"column"}>
        <Grid item={true} style={{ paddingBottom: 10 }}>
          <WorkSearchList
      ...
      <Grid item={true} xs={12}>
        <ExpansionPanel
          ...
          onChange={() => setExpandedUnion(true)}
          elevation={expandedUnion ? 1 : 0}
        >
          <ExpansionPanelSummary
            expandIcon={
              expandedUnion ? null : <Typography>Read more</Typography>
            }
            aria-label="Expand"
        ...
        <ExpansionPanel
          expanded={expandedTempLayoff}
          onChange={() => setExpandedTempLayoff(true)}
          style={{ paddingTop: 15 }}
          elevation={expandedTempLayoff ? 1 : 0}
        >
          <ExpansionPanelSummary
            expandIcon={
              expandedTempLayoff ? null : <Typography>Read more</Typography>
            }
            aria-label="Expand"
            aria-controls="additional-actions1-content"
            id="additional-actions1-header"
